...
public struct BenchmarkReport: Sendable, Codable {
    ...
    /// The fastest tier that still keeps up with live audio and did not fail.
    ///
    ...
    /// committing at all. Accuracy is the tiebreak, not the first filter.
    public var recommendedTier: ModelTier? {
        let usable = runs.filter { $0.failure == nil && $0.rtf > 0 }
        guard !usable.isEmpty else { return nil }
        let live = usable.filter(\.canKeepUpLive)
        // Among models that keep up, prefer the slowest -- it is the most
        // accurate one that still fits the budget.
